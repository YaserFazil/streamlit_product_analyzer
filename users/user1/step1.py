...
print("The 'database.csv' file has been created with only the necessary columns.")
...
def determine_code_type(row):
    ...
    code_upper = row['Code'].upper()  # Convert code to uppercase to simplify checks
...
for code_type in code_types:
    filtered_df = df[df['Code Type'] == code_type].copy()  # Make a copy to avoid SettingWithCopyWarning
...
print("CSV files have been created for each code type, with 'ASIN' and 'FBA' codes capitalized and without 'Code Type' and 'Manual Entry' columns.")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if asin and asin != 'None':  # Check if ASIN is valid and not 'None'
                ...
                print(f"Received 'None' for {code}, treating as a failed attempt.")
                raise ValueError("Received 'None' for ASIN")
        else:
            print(f"Attempt {attempt} failed for {code}. Status code: {response.status_code}, Response: {response.text}")
            raise Exception(f"API request failed with status code {response.status_code}")
    except Exception as e:
        if attempt < max_attempts:
            wait_time = 2 ** attempt  # Exponential backoff
            print(f"Retrying in {wait_time} seconds... (Attempt {attempt} of {max_attempts})")
            time.sleep(wait_time)
            return convert_code_to_asin(code, attempt + 1, max_attempts)
        else:
            print(f"Failed to convert {code} after {max_attempts} attempts. Giving up.")
            return None
...
success_df = df[df['Conversion Status'] == True].drop(columns=['Conversion Status'])
failure_df = df[df['Conversion Status'] == False].drop(columns=['ASIN', 'Conversion Status'])
...
codes_converted_successfully_df = pd.read_csv('codes_converted_successfully.csv')

# Replace 'Code' with 'ASIN' in the 'codes_converted_successfully' DataFrame
codes_converted_successfully_df['Code'] = codes_converted_successfully_df['ASIN']

# Now drop the 'ASIN' column as it's no longer needed
codes_converted_successfully_df = codes_converted_successfully_df.drop(columns=['ASIN'])
...
def fetch_asin_details(asin, api_key, initial_domains=['amazon.ca', 'amazon.com', 'amazon.co.uk', 'amazon.co.jp'], retries=2):
    """Fetch details for a given ASIN with retry logic, cycling through specified domains."""
    print(f"Fetching details for ASIN: {asin}")
    final_data = {}
    domains_to_try = initial_domains.copy()  # Copy the list to avoid altering the original
    ...
    while domains_to_try:
        ...
        for attempt in range(1, retries + 1):
            ...
            if response.status_code == 200:
                ...
                description = data.get('description', '') or data.get('feature_bullets_flat', '')
                final_data['description'] = description

                if data.get('buybox_winner', {}).get('price', {}):
                    final_data['MSRP'] = data.get('buybox_winner', {}).get('price', {}).get('value', '')
                    final_data['currency'] = data.get('buybox_winner', {}).get('price', {}).get('currency', '')

                if data.get('main_image', {}):
                    final_data['image'] = data.get('main_image', {}).get('link', '')

                final_data['link'] = data.get('link', '')

                if final_data.get('link', ''):  # Success with a link
                    print(f"Successfully fetched data for ASIN: {asin} on {current_domain}")
                    return final_data, True
            else:
                print(f"Failed to fetch details for ASIN: {asin} with status code: {response.status_code} on {current_domain}")
    ...
    print(f"Returning partial data after exhausting all domains for ASIN: {asin}")
...
def write_details_to_csv(filename, rows, api_key):
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['Lot', 'Code', 'Consignor', 'Title', 'Description', 'MSRP', 'Currency', 'Auto-Image', 'Product Link'])
        for row in rows:
            asin = row['Code']
...
def main():
    ...
    if failed_asins:
        with open(failed_output_filename, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=['Lot', 'Code', 'Consignor'])
            writer.writeheader()
...
upc_df = pd.read_csv('UPC_codes.csv', dtype=str)  # Ensure codes are read as strings
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                return {
                    ...
                    'MSRP': data['stores'][0].get('price', '') if data.get('stores') else '',
                    'Currency': data['stores'][0].get('currency', '') if data.get('stores') else '',
                    'Auto-Image': data['images'][0] if data.get('images') else '',
                    'Product Link': data['stores'][0].get('link', '') if data.get('stores') else '',
                    'Success': True
                }
            else:
            ...
            print(f"Attempt {attempt_count}: Failed to retrieve data for UPC {upc_code}. Status code: {response.status_code}")
            if attempt_count < 2:  # If this was the first attempt, wait before retrying
...
failed_codes_df.to_csv('upc_failed_details.csv', index=False, columns=['Lot', 'Code', 'Consignor'])

print("Processing completed. Check 'upc_with_details.csv' and 'upc_failed_details.csv' for outputs.")
...
combined_df['Status'] = 'True'  # Add a 'Status' column indicating these are successful entries
combined_df['Manual Entry'] = 'False'  # Add a 'Manual Entry' column with default 'false'

# Load the 'Lot' and 'Consignor' columns from each failed CSV file
codes_failed_df = pd.read_csv('codes_failed_to_convert.csv', usecols=['Lot', 'Consignor'], dtype=str)
upc_failed_df = pd.read_csv('upc_failed_details.csv', usecols=['Lot', 'Consignor'], dtype=str)

try:
    # Try to load the existing 'asin_failed.csv' file
    asin_failed_df = pd.read_csv('asin_failed.csv', usecols=['Lot', 'Consignor'], dtype=str)
...
combined_lot_df = pd.concat([codes_failed_df, upc_failed_df, asin_failed_df], ignore_index=True).drop_duplicates(subset=['Lot']).reset_index(drop=True)
combined_lot_df['Status'] = 'False'  # Add a 'Status' column indicating these are failed entries
combined_lot_df['Manual Entry'] = 'False'  # Indicate these are not manual entries
...
manual_entry_df = pd.read_csv('MANUAL-ENTRY_codes.csv', usecols=['Lot', 'Consignor'], dtype=str)
...
missing_cols = [col for col in combined_df.columns if col not in manual_entry_df.columns and col != 'Status' and col != 'Manual Entry']
for col in missing_cols:
    manual_entry_df[col] = ''  # Add empty data for missing columns

# Append MANUAL-ENTRY lots to the combined DataFrame
final_df = pd.concat([combined_df, combined_lot_df, manual_entry_df], ignore_index=True)
...
print("The files have been combined into 'items.csv' successfully, with status and manual entry indicated for each entry.")
...
manual_entries = manual_entries[['Lot', 'Title', 'Description', 'MSRP', 'Currency']]
...
updated_items = pd.merge(df_items, manual_entries, on='Lot', how='left', suffixes=('', '_update'))
...
for column in ['Title', 'Description', 'MSRP', 'Currency']:
    updated_items[column] = updated_items[f'{column}_update'].combine_first(updated_items[column])
...
print("items.csv has been updated with manual entry details from database_export.csv.")
...
def remove_html_css(text):
    ...
    text = re.sub(r'{[^}]+}', '', text)  # Remove inline CSS and any remaining HTML/CSS artifacts
...
items_df['Description'] = items_df['Description'].apply(lambda x: remove_html_css(x))
items_df['Description'] = items_df['Description'].apply(lambda x: remove_emojis(x))
...
database_export_df = pd.read_csv('database_export.csv', usecols=['Lot', 'Auto-Image'], dtype=str)
...
for index, row in items_df.iterrows():
    ...
    if pd.isna(row['Auto-Image']) or row['Auto-Image'].strip() == '':
        ...
        auto_image_value = database_export_df.loc[database_export_df['Lot'] == row['Lot'], 'Auto-Image'].values
        # If a matching 'Lot' is found and it has a non-empty 'Auto-Image' value, update items_df
        if len(auto_image_value) > 0 and pd.notna(auto_image_value[0]) and auto_image_value[0].strip() != '':
...
print("The 'items.csv' file has been updated with 'Auto-Image' values from 'database_export.csv'.")


# List of files to be deleted
files_to_delete = [
    "ASIN_codes.csv", "asin_details.csv", "asins.csv", "asin_failed.csv", 
    "codes_converted_successfully.csv", "codes_failed_to_convert.csv", 
    "FBA_codes.csv", "UPC_codes.csv", "UPC_codes_trimmed.csv", 
    "upc_failed_details.csv", "upc_with_details.csv", "database.csv", "MANUAL-ENTRY_codes.csv"
]
