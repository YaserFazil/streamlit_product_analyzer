...
def start_step1(username):
    with st.status("Loading step 1 process...", expanded=True) as status:
        try:
            st.write("Loading database_export.csv file...")
            time.sleep(3)
            ...
            def fetch_asin_details(
                ...
                while domains_to_try:
                    current_domain = domains_to_try.pop(
                        0
            ...
            def write_details_to_csv(filename, rows, api_key):
                ...
                st.write("Starting to write details to CSV.")
                failed_asins = []
                ...
                st.write("Finished writing details to CSV.")
                return failed_asins

            def main():
                input_filename = f"./users/{username}/uploads/asins.csv"
                output_filename = f"./users/{username}/uploads/asin_details.csv"
                failed_output_filename = f"./users/{username}/uploads/asin_failed.csv"
                ...
                st.write("Reading ASIN codes from CSV.")
                rows = []
                with open(
                ...
                st.write(f"Total rows read: {len(rows)}")
                ...
                if failed_asins:
                    ...
                    st.write(f"Failed ASIN details written to {failed_output_filename}")
            ...
            st.write("Updated items.csv has been saved as items.csv")
