    ...
    with st.status("Loading step 3 process...", expanded=True) as status:
        try:
            ...
            df["Description 1"] = df["Title"].apply(
                lambda x: f"[[[Full Title: {trim_and_ellipsis(str(x), 220)}]]] ~~"
            )
            ...
            df["Description 2"] = df["Description 2"].apply(
                lambda x: f"[[[Notes: {str(x) if not pd.isnull(x) and x.strip() != '' else 'N/A'}]]] ~~"
            )

            # Rename 'Condition' to 'Description 3' and update formatting
            df.rename(columns={"Condition": "Description 3"}, inplace=True)
            df["Description 3"] = df["Description 3"].apply(
                lambda x: f"[[[Item Condition: {x}]]] ~~"
            )
