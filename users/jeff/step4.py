        ...
        try:
            ...
            for _, row in full_data_df.iterrows():
                ...
                for code_type in ["Code-ASIN", "Code-FBA", "Code-UPC"]:
                    if pd.notnull(row[code_type]) and (
                        row[code_type] not in database_df[code_type].values
                    ):
                        unique_asin_or_fba_present = True
                        break

                # Initialize a flag to track if any image is not in S3
                any_image_not_in_s3 = any(
                    pd.notnull(row[col]) and "aucwarehouse" not in row[col]
                    for col in image_columns
                )

                # Proceed if the title is new or if there's a unique ASIN or FBA code
                if (
                    unique_asin_or_fba_present
                    or (row["Title"] not in database_df["Title"].values)
                ) and any_image_not_in_s3:
                    print(f"Processing item: {row['Title']}")
                    st.write(f"Processing item: {row['Title']}")
                    ...
                    for col in image_columns:
                        if col.startswith("Online Image"):
                            ...
                            and "aucwarehouse" not in row[col]
                        ):
            ...
            if items_to_add_df.empty:
                ...
                empty_df = pd.DataFrame(
                    columns=database_df.columns
                )  # Use the correct DataFrame variable name here
